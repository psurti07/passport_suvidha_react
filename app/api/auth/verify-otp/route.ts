...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await axiosServer.post<VerifyOtpApiResponse>('/otp/verify', {
    ...
    if (axios.isAxiosError(error)) {
      ...
      const message = externalMessage || 'An error occurred during OTP verification.';
      // Include details in the error response if possible
      const responseData = {
        message,
        mobile_number: (error.config?.data ? JSON.parse(error.config.data).mobile_number : undefined),
