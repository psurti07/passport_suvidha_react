...
export function Navbar() {
  ...
  const navLinks = [
    ...
    { href: "/services", label: "Services", ariaLabel: "View our services" },
    {
      href: "/requirements",
      label: "Requirements",
      ariaLabel: "Check passport requirements",
    },
    {
      href: "/locations",
      label: "Locations",
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 lg:px-6 flex h-16 sm:h-20 items-center justify-between gap-4">
        ...
        {/* Desktop Navigation */}
        <nav
          className="hidden md:flex items-center gap-4 lg:gap-5 xl:gap-8"
          role="navigation"
        ...
        >
          {navLinks.map((link) => (
            <Link
              ...
              className={cn(
                "text-xs lg:text-sm font-medium transition-all duration-200 relative py-1",
                isActive(link.href)
                  ? "text-gold"
        ...
        <div className="hidden md:flex items-center gap-2 lg:gap-3">
          <Button
            variant="outline"
            className="bg-transparent border-white/20 text-white hover:bg-white/10 hover:text-gold rounded-full text-xs lg:text-sm px-3 lg:px-4 transition-all duration-200"
            asChild
          >
          ...
          <Button
            className="bg-gradient-to-r from-gold to-gold/80 text-navy hover:opacity-90 rounded-full modern-button text-xs lg:text-sm px-4 lg:px-5 transition-all duration-200 transform hover:scale-105"
            asChild
          >
            <Link
              href="/apply-passport"
            ...
            >
              ...
              <div className="pt-4 flex flex-col gap-3">
                ...
                <Link
                  ...
                  aria-label="Start passport application"
                >
                  <Button className="w-full bg-gradient-to-r from-gold to-gold/80 text-navy hover:opacity-90 rounded-full modern-button transform hover:scale-105 transition-all duration-200">
                    Apply Now
                  </Button>
                </Link>
              </div>
